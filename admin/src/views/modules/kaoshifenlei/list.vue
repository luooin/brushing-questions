		...
		<template v-if="showFlag">
				...
				<el-row
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label
							...
							class="item-label">试题库分类</label>
						<el-input v-model="searchForm.kaoshifenlei" placeholder="试题库分类" @keydown.enter.native="search()"
							clearable></el-input>
					</div>
			...
			<div
				...
				<el-table class="tables" :stripe='true'
					...
					:border='true' v-if="isAuth('kaoshifenlei','查看')" :data="dataList" v-loading="dataListLoading"
					@selection-change="selectionChangeHandler">
					<el-table-column :resizable='true' type="selection" align="center" width="50"></el-table-column>
					<el-table-column :resizable='true' :sortable='true' label="序号" type="index" width="50" />
					<el-table-column :resizable='true' :sortable='true' prop="kaoshifenlei" label="试题库分类">
						<template slot-scope="scope">
							{{scope.row.kaoshifenlei}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('kaoshifenlei','修改') " type="success"
							...
							<el-button class="del" v-if="isAuth('kaoshifenlei','删除') " type="primary"
			...
			<el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle"
				...
				:layout="layouts.join()" :total="totalPage" prev-text="上一页 " next-text="下一页 "
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if (this.searchForm.kaoshifenlei != '' && this.searchForm.kaoshifenlei != undefined) {
					params['kaoshifenlei'] = '%' + this.searchForm.kaoshifenlei + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				console.log(user)
				this.$http({
					url: "kaoshifenlei/page",
					method: "get",
					params: params
				}).then(({
					data
			...
			download(file) {
				...
				axios.get(this.$base.url + 'file/download?fileName=' + arr, {
					...
					axios.get((location.href.split(this.$base.name).length > 1 ? location.href.split(this.$base
						.name)[0] : '') + this.$base.name + '/file/download?fileName=' + arr, {
						headers: {
							token: this.$storage.get('Token')
						},
						responseType: "blob"
			...
			preClick(file) {
				...
				window.open((location.href.split(this.$base.name).length > 1 ? location.href.split(this.$base.name)[0] +
					this.$base.name + '/' + file : this.$base.url + file))
			},
			kaoshifenleistatusChange(e, row) {
				if (row.status == 0) {
					row.passwordwrongnum = 0
				}
				this.$http({
					url: 'kaoshifenlei/update',
					method: 'post',
					data: row
				}).then(res => {
					if (row.status == 1) {

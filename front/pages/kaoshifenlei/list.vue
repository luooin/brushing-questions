		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#F2F2F2","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx 0","background":"none","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 0 0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"10rpx","background":"#F8F8F8","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.kaoshifenlei" type="text" placeholder="考试分类" ></input>
				</view>
			...
			<view class="uni-product-list" :style='{"padding":"10rpx 20rpx","margin":"0 auto","borderRadius":"10rpx","flexWrap":"wrap","background":"#fff","flex":"1","display":"flex","width":"calc(100% - 160rpx)","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"none","padding":"0 0 10rpx","margin":"0 0 20rpx","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240104/f251fe8ff5ac4c80afbbaad725ab3bbb.png)","display":"flex","width":"48%","backgroundSize":"100% 100%","backgroundPosition":"center center","backgroundRepeat":"no-repeat","justifyContent":"center","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"11"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshifenlei','修改')) || (!userid && isAuthFront('kaoshifenlei','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshifenlei','删除')) || (!userid && isAuthFront('kaoshifenlei','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"8rpx 20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"11"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshifenlei','修改')) || (!userid && isAuthFront('kaoshifenlei','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshifenlei','删除')) || (!userid && isAuthFront('kaoshifenlei','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.kaoshifenlei){
					params['kaoshifenlei'] = '%' + this.searchForm.kaoshifenlei + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`kaoshifenlei`, params);
                } else {
                    res = await this.$api.list(`kaoshifenlei`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kaoshifenlei', JSON.stringify([id]));
							this.$utils.msg('删除成功');
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.search()
			...
			async search(){
				...
				if(this.searchForm.kaoshifenlei){
					searchForm['kaoshifenlei'] = '%' + this.searchForm.kaoshifenlei + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`kaoshifenlei`, searchForm);
                } else {
                    res = await this.$api.list(`kaoshifenlei`, searchForm);
                }
				// 如果是第一页数据置空
